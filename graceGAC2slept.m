function varargout=graceGAC2slept(Dataproduct,TH,XY_buffer,Lwindow,phi,theta,omega,J,units,forcenew)
...
if ~isstr(TH) && length(TH)==1 % POLAR CAPS
    ...
    if lp
        fnpl=sprintf('%s/graceGAC2slept-%s-CAP-%i-%i-%i-%s.mat',...
            ddir2,Dataproduct,TH,Lwindow,J,units);
    elseif bp
        fnpl=sprintf('%s/graceGAC2sleptbl-%s-CAP-%i-%i-%i-%i-%s.mat',...
    ...
    if lp
        fnpl=sprintf('%s/graceGAC2slept-%s-%s-%i-%i-%s.mat',...
            ddir2,Dataproduct,h,Lwindow,J,units);
    elseif bp
        fnpl=sprintf('%s/graceGAC2sleptbl-%s-%s-%i-%i-%i-%s.mat',...
...
if exist(fnpl,'file')==2 && forcenew==0
     load(fnpl)
     disp(sprintf('%s loaded by GRACEGAC2SLEPT',fnpl))
else
    ...
    [potcoffs,thedates]=graceGAC2plmt(Pcenter,Rlevel,units,0);
