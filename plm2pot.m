...
%
% Last modified by
%   fjsimons-at-alum.mit.edu, 12/10/2021
%   kengourley-at-email.arizona.edu, 08/29/2018
%   charig-at-princeton.edu, 05/14/2015
%   kwlewis-at-princeton.edu, 02/23/2012
...
function lmcosi = plm2pot(lmcosi, r, GM, a, wat, wit)
    ...
    switch wat
        ...
        case 4
            ...
            rhoave = 5517;
            fprintf('Average Earth density used is %i kg/m^3\n', rhoave)
    ...
    switch wit
        case 'WGS84'
            ...
            for l = [barC2n(:, 2)]'
                ...
                fprintf('Adjusting degree %2i with degree %2i\n', lmcosi(were, 1), barC2n(l / 2, 2))
            end
    ...
    if lmcosi(1) == 2
        ...
        lmcosi = [0 0 GMr 0 zeros(1, size(lmcosi, 2) - 4); % Degree 0
                  1 0 0 0 zeros(1, size(lmcosi, 2) - 4); % Degree 1 order 0
                  1 1 0 0 zeros(1, size(lmcosi, 2) - 4); % Degree 1 order 1
                  lmcosi]; % Then the rest
        % Preserve dimension of scaling factor when calculating geoid
        fact = [ones(3, size(lmcosi, 2) - 4); fact];
    elseif lmcosi(1) == 1 && lmcosi(1, 2) == 0
        lmcosi = [0 0 GMr 0 zeros(1, size(lmcosi, 2) - 4); % Degree 0
                  lmcosi]; % Then the rest
        fact = [ones(1, size(lmcosi, 2) - 4); fact];
    end
