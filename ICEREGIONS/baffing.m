  ...
  if exist(fnpl,'file')==2 
    ...
    if buf ~=0
      ...
      if exist(fnpl2,'file')==2
        ...
        % Now subtract a smaller Baffin from Ellesmere
        ...
        % Periodize our way
        LonB(LonB<0) = LonB(LonB<0)+360;
        % Now subtract the thing next door from Baffin so we don't overlap Ellesmere
        [x,y] = polybool('subtraction',LonB,LatB,x1,y1);  
        
        % If we get too big, Greenland intrudes, so subtract a nearby 
        % Greenland as well.
        if buf > 1.5
            XY3 = greenland(10,buf);
            % Subtract from Greenland the minimum baffin we want to keep
            XY4 = baffing(10,1.5);
            [xg,yg] = polybool('subtraction',XY3(:,1),XY3(:,2),XY4(:,1),XY4(:,2));
            % Then subtract this from the 'buf' Baffin
            [x,y] = polybool('subtraction',x,y,xg,yg);   
            XY = [x y];
        end
...
elseif strcmp(res,'demomake')
      ...
      % coordinate files, since there are dependencies.
