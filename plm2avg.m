function varargout = plm2avg(lmcosi, dom)
    ...
    if xver == 1
        ...
        fprintf('Area check...  PLM2AVG A: %6.7f ; SPHAREA A: %6.7f ; AREAINT A: %6.7f\n', ...
            miniK(1), A1, A2)
    end
...
%% Demos
function vOut = rundemo1(nArgout)
    ...
    L = 40;
    degres = [];
    [~, ~, ~, ~, ~, lmcosi] = geoboxcap(L, dom, [], degres, 1);
    [Int, A, miniK, XY] = plm2avg(lmcosi, dom);
    plotplm(lmcosi, [], [], 4, degres);
    colorbar
    % Really dumb thing
...
function rundemo2
    ...
    XY(:, 1) = XY(:, 1) - 360 * (XY(:, 1) > 360); %#ok<NODEF>
